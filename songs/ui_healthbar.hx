...
function postCreate() {
    healthBar.visible = healthBarBG.visible = iconP1.visible = iconP2.visible = true;

    gorefieldhealthBarBG = createBGBar((stage != null && stage.stageXML != null && stage.stageXML.exists("healthBarColor")) ? stage.stageXML.get("healthBarColor") : "cut");

{
        gorefieldhealthBar = new FlxBar(0, 0, FlxBarFillDirection.RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
        'health', 0, 2);
    }
    ...
    gorefieldhealthBar.y = (gorefieldhealthBarBG.y = FlxG.height * 0.89) + (PlayState.SONG.meta.name.toLowerCase() == "captive" ? 0 : 34);
